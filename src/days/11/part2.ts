...
self.onmessage = async (e) => {
  ...
  if (command === "START") {
    ...
    const firstSeatAt = (rows, y, x, dy, dx) => {
      while (y >= 0 && y < lines.length && x >= 0 && x < rowLength) {
        // debugger;
        y += dy;
        x += dx;
        if (seatAt(rows, y, x) !== ".") {
          return seatAt(rows, y, x);
        }
      }
      return ".";
    };
    const adjacentCount = (rows, y, x) =>
      (firstSeatAt(rows, y, x, -1, 0) === "#" ? 1 : 0) +
      (firstSeatAt(rows, y, x, +1, 0) === "#" ? 1 : 0) +
      (firstSeatAt(rows, y, x, -1, -1) === "#" ? 1 : 0) +
      (firstSeatAt(rows, y, x, -1, +1) === "#" ? 1 : 0) +
      (firstSeatAt(rows, y, x, +1, -1) === "#" ? 1 : 0) +
      (firstSeatAt(rows, y, x, +1, +1) === "#" ? 1 : 0) +
      (firstSeatAt(rows, y, x, 0, -1) === "#" ? 1 : 0) +
      (firstSeatAt(rows, y, x, 0, +1) === "#" ? 1 : 0);
